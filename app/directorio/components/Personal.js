...
const Personal = () => {
    ...
    const handleClick = () => {
        ver === 5?
        setVer(personal.length)
        :
        setVer(5);
    }
    const personal = empresas.filter(empresa => (empresa.rubro === "personal")).sort((a, b) => a.titulo.localeCompare(b.titulo));
    return (
        <div id='personal'>
            {personal.length >= 1 ? 
                <div id='personal' className='flex flex-col bg-[#75C495]'>
                    ...
                    <h3 className='text-3xl lg:text-4xl font-bold text-center mt-2 mb-6 font-titulos'>Cosmética y cuidado</h3> 
                    <div className="flex justify-center gap-4 mb-2 flex-wrap px-2 pb-10">
                        <div className="flex justify-center gap-4 flex-wrap">
                            {personal.slice(0, ver).map((empresa) => (
